...
```
curl -X POST -H "Content-type: application/json" -d '{"title":"演示任务 01","remark":"备注","cover":{"templateCode":"zCEPaLgNgtczovLGL0W1Q","data":{"param_name1":{"value":"","color":""}}},"body":{"templateCode":"yxS2ORvMviyCrhrDn-gNb"}}' "http://localhost:3000/send/task/add"
```
...
```
curl "http://localhost:3000/send/task/send?code=3Pkm3MVPY_Kt5ooC4C5qW"
```
...
```
curl -X POST -H "Content-type: application/json" "http://localhost:3000/send/message/add?task=3Pkm3MVPY_Kt5ooC4C5qW" -d '{"receivers":["user1","user2"],"cover":{"data":[{"user_name":"name1"},{"user_name":"name2"}]},"body":{"data":[{"user_name":"name1"},{"user_name":"name2"}]}}'
```
